                            ...
                            <div class="card">
                                <div class="card-body">
                                    <h5 class="text-center">
                                        Admitted Students for <?php echo $_SESSION['adm_session']?> Academic Session
                                        <span style="float:right;">
                                            <form method="post" action="<?php echo site_url('registration/change_session')?>">
                                        ...
                                        <table class="table table-hover js-basic-example dataTable table-striped table_custom border-style spacing5" id="myTable1">
                                            ...
                                            <tbody>
                                                ...
                                                foreach ($adm_list as $row) { ?>
                                                    <tr>
                                                        <td><?php echo $i++; ?></td>
                                                        <td><?php echo '<i class="fa fa-circle" style="color: '.($row->pnumber != 'Unassigned' ?'green':'red' ).'"></i>'.' '. ($row->pnumber) ?></td>
                                                        <td><?php echo strtoupper($row->jamb_no) ?></td>
                                                        <td><?php 
                                                        echo //'<img src="'.base_url('passport/' . $row->passport).'" style="width:40px; border-radius:50%"> ' .
                                                        strtoupper($row->surname)." ".ucwords(strtolower($row->firstname." ".$row->othername)); 
                                                        ?></td>
                                                        <td><?php echo $row->gender; ?></td>
                                                        <td><?php echo $row->entrymode; ?></td>
                                                        <td><?php echo $row->prog_abbr; ?></td>
                                                        <td>
                                                            <a class="btn btn-info" href="<?php echo site_url('registration/admin_view/'.md5(time().rand()).'/' . $row->user_id).'/'.md5(rand()); ?>">
