	    ...
	    <tr>
	        <td style="width:77%; vertical-align:top;">
	            ...
	            <table>
    				...
    				<tr>
    					<td class="value" colspan="2" style="font-size:19px; padding-top:4px;"><?php echo $row->pnumber; ?></td>
    				</tr>
			...
			<tr>
				<td class="label" style="padding-left:3.5px; padding-top:3px;">
				    ...
				    <span style="float:left; padding:4px; 1px 3px 1px; font-weight:normal; font-size:13px">
				        <?php echo $row->entrymode == "PG" ? "Postgraduate" : "Undergraduate"; ?><br>
				        Valid Dates: 
				        <?php
				            $start = substr($row->session_admitted, 0, 4);
				            
				            if($row->entrymode != "PG"){
				                $end =  $row->entrymode == "UTME" ? substr($row->duration, -3) : substr($row->duration_de, -3) ;
    				            $end = str_replace("(", "", $end);
    				            $end = str_replace(")", "", $end);
				            }else{
				                $end =  substr($row->duration, -3) ;
    				            $end = str_replace("(", "", $end);
    				            $end = str_replace(")", "", $end);
    				            $end /= 2;
				            }
				...
				<td class="label" style="padding:3px 2px 3px 0px; color:<?php echo $row->entrymode == "PG" ? "blue" : "red"; ?>; font-weight:900; font-size:22px;">STUDENT</td>
			</tr>
	    </tr>
	</table>
</div>
